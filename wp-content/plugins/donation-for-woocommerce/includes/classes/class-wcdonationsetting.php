...
class WcdonationSetting {
	...
	public function has_bought_items( $product_id = 0  ) {
	...
	public function wc_donation_modify_column_names ( $columns ) {
		...
		$columns['campaign_name'] = __('Campaign Name', 'wc-donation');		
		$columns['amount'] = __('Amount', 'wc-donation');
		$columns['total_donations'] = __('No. of Donations', 'wc-donation');
	...
	public function get_products_without_donation( $query ) {
		...
		if ( $query->is_main_query() && 'product' === $query->query[ 'post_type' ] ) :
	...
	private function update_product_meta ( $post_id, $prod_id ) {
		...
		if ( 'disabled' == $RecurringDisp || empty($RecurringDisp) ) {
			...
			if ( 'user' == $RecurringDisp || empty($RecurringDisp ) ) {
				update_post_meta( $prod_id, '_subscription_custom_length', $length );
			}	
		}
		...
		update_post_meta( $prod_id, '_regular_price', '' );
		...
		update_post_meta( $prod_id, '_sku', 'wc-donate-' . $prod_id );
