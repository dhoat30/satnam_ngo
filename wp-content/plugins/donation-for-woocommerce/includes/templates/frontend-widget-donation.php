...
if ( ! empty( $donation_product ) ) {
	...
	<style>
		:root {
			--wc-wdg-bg-color: #<?php echo esc_html($donation_button_color); ?>;
			--wc-wdg-txt-color: #<?php echo esc_html($donation_button_text_color); ?>;
		}
	</style>
	<div class="wc-donation-in-action">
		<label for="donation-price"><?php echo esc_html( __( $donation_label, 'wc-donation' ) ); ?></label>
		<div class="in-action-elements <?php echo esc_attr($display_donation_type); ?>">
			<?php
			if ( 'before' === $where_currency_symbole && ( 'select' === $display_donation_type || 'free-value' === $display_donation ) ) {
				?>
				<style>
					...
					.widget_wc-donation-widget .wc-donation-in-action .in-action-elements span {
						...
						background: #<?php echo esc_html($donation_button_color); ?>;
						color: #<?php echo esc_html($donation_button_text_color); ?>;						
			...
			<?php } ?>
			<?php
			if ( 'after' === $where_currency_symbole && ( 'select' === $display_donation_type || 'free-value' === $display_donation )  ) {
				?>
				<style>
					.widget_wc-donation-widget .wc-donation-in-action .in-action-elements #wc-donation-f-donation-value {
						margin-right: 0;
