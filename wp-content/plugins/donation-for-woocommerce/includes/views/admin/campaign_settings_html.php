...
<div class="select-wrapper display-option">
	...
	foreach ( WcDonation::DISPLAY_DONATION() as $key => $value ) { 
		...
		echo '<label for="' . esc_attr($key) . '"><input type="radio" id="' . esc_attr($key) . '" name="wc-donation-amount-display-option" value="' . esc_attr($key) . '" ' . esc_attr($checked) . ' >' . esc_attr( $value ) . '</label>';
	}
	?>
</div>
